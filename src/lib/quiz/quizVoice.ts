 ...
 *      Austrian-flavored cartoon voice). Reuses the WebSocket plumbing
 ...
 *      no API key required, sounds robotic. Picks a German-accented voice if
...
- Voice: warm, playful, slightly silly cartoon-host energy with a
  cheerful Austrian-flavored accent — sing-song intonation, big
  open vowels. Cheerful upward lifts at phrase ends.
- This applies even when the line you receive is in Hebrew or another
  language: keep the Austrian *accent and prosody*, but the *words*
...
- Voice: warm, playful, cartoon-host energy with a cheerful
  Austrian-flavored accent on top of Hebrew speech — sing-song
  intonation, big open vowels, cheerful upward lifts.
...
class BrowserTtsQuizVoice implements QuizVoice {
   ...
   *  language. A German-accented voice adds a hint of Austrian flavor
   *  for English output; for Hebrew we use the best he-IL voice. */
  private pickVoice(): void {
    ...
    if (this.lang === "he") {
      ...
      // Prefer a German/Austrian-accented English voice if one is
      // installed, else any en-* voice, else the default.
      this.chosenVoice =
        voices.find(v => /german|deutsch|austria/i.test(v.name) && /^en/i.test(v.lang)) ??
