...
const TRIP_FACTS = {
  ...
  planNotes: [
    ...
    "CONFIRMED with the hotel: the Stacherhof has NO air conditioning. AC was the family's original must-have, so be practical about it if it comes up: shutters/curtains closed through the afternoon, windows open once it cools, a clip-on fan for the kids' room, and the coolest activities (lakes, water parks, shaded gorges) on the hottest days.",
    ...
    "THE PLAN IS HILA'S. She wrote the day-by-day and it is deliberately play-park / lift / water heavy, which suits Gili (3) and Tom (1.5). Her rhythm every day: morning anchor from ~08:30, lunch ~12:30, a NAP IN THE CAR during the transfer, gentler afternoon, home ~17:00. Respect that rhythm in any suggestion you make.",
    "Measured drive corrections the family should know (Hila's own estimates were optimistic on these): Fügen→Hexenwasser Söll is 52 min (not 35), Fügen→Triassic Park Waidring 76 min (not 55), Zell am Ziller→Hintersteiner See 65 min (not 40), Lauserland→Freising 2 h (not 1.5). Conversely Hohe Salve→Salvenaland is only 4 min, and Rofan→Pertisau only 6 min.",
    "Pillersee had to move: it is ~20 min from Triassic Park Waidring but ~2 hours from the Zillertal base, so it sits on the Triassic day (Aug 17), not with Mayrhofen as originally drafted.",
    "Sat Aug 15 is Mariä Himmelfahrt, a national holiday: Rattenberg's glass shops will very likely be shut that afternoon, so it's a stroll-and-café visit unless they swap it to another day. Do the supermarket run on Friday the 14th.",
    "Swarovski Kristallwelten is the designated RAINY-DAY backup (Hila's own call) — not on a fixed day. Innsbruck + the Alpenzoo is available as a swap if they want a city-and-animals day, but the park days suit the kids better.",
    "Day 9 (Aug 19) is the drive out: check out, a last morning at Lauserland in the Alpbachtal, then ~2 h to a hotel in the Freising / Munich-airport area (still being booked). The Chiemsee lido at Übersee is an optional leg-stretch on that drive, 5 min off the A8. Day 10 (Aug 20) is Olympiapark München in the morning — but leaving the park by ~11:15 is the safe call, because the Sixt return is a hard 13:00 and it's a 28 min drive plus fuel.",
    "The base is in the ZILLERTAL, not the Alpbachtal. Measured drive times from Fügen: Spieljoch 6 min, Zell am Ziller (Zillertal Arena) 15 min, Mayrhofen/Sennerei 21 min, Rofan 22 min, Lauserland (Alpbach) 24 min, Kundler Klamm 26 min, Hopfgarten (Hohe Salve/Salvenaland) 40 min, Söll 52 min, Triassic Park Waidring 76 min, Munich Airport 1 h 50.",
    ...
    "Achensee (the Pertisau lido, the Spielplatz am Achensee and the lake steamer) is the Day 3 (Aug 13) afternoon, paired with the Rofan cable car that morning.",
    "Departure day (Aug 20): there IS a morning plan now — Olympiapark München — but the car must be back at Munich Airport by 13:00 for the 16:30 flight, so leaving the park by ~11:15 is the safe call.",
